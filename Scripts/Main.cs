...
public partial class Main : Node2D
{
	...
	private readonly Dictionary<CharacterBody2D, Area2D> _ladderDetectors = new();
	...
	private void SimulatePlayer(CharacterBody2D player, double delta, uint worldLayer)
	{
		...
		if (_ladderDetectors.TryGetValue(player, out var detector))
		{
			foreach (var area in detector.GetOverlappingAreas())
			{
				if (area.IsInGroup("ladder"))
				{
					isClimbing = true;
					break;
				}
	...
	private CharacterBody2D CreatePlayer(Vector2 position, Color color, uint layer, uint mask, string nodeName)
	{
		...
		ladderDetector.AddChild(new CollisionShape2D
		{
			Shape = new RectangleShape2D { Size = new Vector2(24, 40) }
		});
		player.AddChild(ladderDetector);
		_ladderDetectors[player] = ladderDetector;
